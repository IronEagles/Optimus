...
TFileHandle flightLDatFH;
//============================================================================
// remove old file, open new one and write the 2-D array header to it.
//
int open_flightL_record() {
   TFileIOResult nIoResult;
   const string sFileName = "flightL.txt";
   int nFileSize = 15 * 1024;
   ClearTimer(T4);

   Delete(sFileName, nIoResult);
   OpenWrite(flightLDatFH, nIoResult, sFileName, nFileSize);
   if (nIoResult != 0) {
      wait10Msec(50);
      StopAllTasks();
   }
   WriteText(flightLDatFH, nIoResult, "int flightL[][8]={\n");
   wait10Msec(5);
   return(0);
}
//============================================================================
...
//
int close_flightL_record() {
   TFileIOResult nIoResult;

   WriteText(flightLDatFH, nIoResult, "{-1,0,0,0,0,0");
   WriteText(flightLDatFH, nIoResult, ",0,0}};\n");
   Close(flightLDatFH, nIoResult);
...
void update_flightL_record() {
   ...
   WriteText(flightLDatFH, nIoResult, str);
